...
switch( $action ) {
    ...
    case 'Save Changes':
        ...
        if (empty($modified_task)) {
            $errors[] = 'You did not give us any task to modify';
        } elseif ($i === NULL || $i === FALSE) {
            $errors = 'We cannot modify the task';
        } else {
    ...
    case 'Promote Task':
        $task_index = filter_input(INPUT_POST, 'taskid', FILTER_VALIDATE_INT);
        if ($task_index === NULL || $task_index === FALSE) {
            $errors[] = 'Nothing to promote.';
        } elseif ($task_index == 0) {
            $errors[] = 'This task is already number 1!';
        } else {
